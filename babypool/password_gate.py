...
Unset / empty ``APP_PASSWORD`` = gate OFF (local dev): the app serves the page
without a login.

This module holds the stateless helpers and the in-memory per-IP failed-login
rate limiter; the routes + ``before_request`` gate live in :mod:`.web`. It is a
faithful copy of the taste-twin gate (same posture across all of Graham's apps)
— kept as its own module so the security-critical bits live in one small,
reviewable place.
"""
...
def client_ip() -> str:
    ...
    # Trusting CF-Connecting-IP is safe ONLY because this app is reachable
    # exclusively via the Cloudflare tunnel (no host port mapping) — off-tunnel
    # that header would be attacker-spoofable. It only feeds the rate-limiter,
    # never authorization; the constant-time shared-password compare is the gate.
...
class LoginRateLimiter:
    ...
    of bad guesses locks that IP for ~15 minutes). Process-local — fine for the
    single-box deployment; a restart clears it.
    """
